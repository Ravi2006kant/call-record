...
class _RecordingsScreenState extends State<RecordingsScreen>
  ...
  /// Full scan: walks the whole internal storage looking for audio files that
  /// sit in a folder whose path contains "call". Shows WHERE recordings are kept.
  ...
  Future<Recording?> _toRecording(File f, {required bool requireHint}) async {
    ...
    final folder = path.substring(0, path.length - name.length - 1);
    if (requireHint) {
      // Judge by the FOLDER only, so songs called "Call Me..." are not matched
